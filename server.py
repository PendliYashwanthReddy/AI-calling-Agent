...
from db import (
    SENSITIVE_KEYS, ENV_CONFIG_KEYS, cancel_appointment, clear_errors, create_campaign, delete_campaign,
...
# override=False → real VPS environment variables ALWAYS take precedence over a
# local .env. On the VPS there is no .env file, so config comes purely from the
# process environment, which is the single source of truth for all credentials.
...
async def eff(key: str) -> str:
    """Effective config value — read ONLY from the process/VPS environment."""
...
async def api_save_settings(req: SettingsRequest):
    """
    Only application-data settings (e.g. ENABLED_TOOLS) are persisted.
    Credential/service keys are rejected — VPS environment variables are the
    single source of truth and can only be changed on the server itself.
    """
    rejected = [k for k in req.settings if k in ENV_CONFIG_KEYS]
    app_only = {
        k: v for k, v in req.settings.items()
        if v is not None and v != "" and k not in ENV_CONFIG_KEYS
    }
    if app_only:
        await save_settings(app_only)
    return {
        "status": "saved",
        "count": len(app_only),
        "rejected": rejected,
        "note": ("Credentials are read only from VPS environment variables and "
                 "were not modified.") if rejected else "",
    }
...
async def api_setup_trunk():
    ...
    try:
        ...
        try:
            ...
            await lk.aclose()
        # We do NOT write this anywhere. The agent worker (a separate process)
        # reads OUTBOUND_TRUNK_ID from its own environment, so the env var is the
        # single source of truth: set it on the VPS and restart to start dialing.
        return {
            "status": "created",
            "trunk_id": trunk_id,
            "note": (f"Set OUTBOUND_TRUNK_ID={trunk_id} in your VPS environment "
                     "variables and restart the service to enable outbound calls."),
        }
    except Exception as exc:
