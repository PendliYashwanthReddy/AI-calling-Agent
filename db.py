...
# ---------------------------------------------------------------------------
# SINGLE SOURCE OF TRUTH = process / VPS environment variables.
# Every key below is read ONLY from os.environ — never persisted to or read
# from the database. The dashboard can DISPLAY their status but cannot change
# them; to change a value you set the VPS env var and restart the service.
# ---------------------------------------------------------------------------
ENV_CONFIG_KEYS = {
    ...
    "AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_REGION", "AWS_BUCKET_NAME", "S3_ENDPOINT",
    "CALCOM_API_KEY", "CALCOM_EVENT_TYPE_ID", "CALCOM_TIMEZONE",
    "SUPABASE_URL", "SUPABASE_SERVICE_KEY", "DB_PATH",
}

# Application DATA stored in Supabase (NOT environment configuration).
APP_DATA_SETTING_KEYS = {"system_prompt", "ENABLED_TOOLS"}
...
async def _adb():
    ...
    # Prefer the stable public async factory; fall back to the internal path.
    try:
        from supabase import acreate_client
        return await acreate_client(url, key)
    except ImportError:
        from supabase._async.client import create_client as _async_create
        return await _async_create(url, key)
...
async def get_all_settings() -> dict:
    """
    Return configuration status. ALL credential/service keys are read straight
    from the process environment (VPS env vars) — the database is never consulted
    for these. Only ENABLED_TOOLS (application data) is read from Supabase.
    """
    DISPLAY_KEYS = [
        "LIVEKIT_URL", "LIVEKIT_API_KEY", "LIVEKIT_API_SECRET",
        "GOOGLE_API_KEY", "GEMINI_MODEL", "GEMINI_TTS_VOICE", "USE_GEMINI_REALTIME",
    ...
    for k in DISPLAY_KEYS:
        ...
        if k in SENSITIVE_KEYS:
            out[k] = {"value": "", "configured": bool(env_val), "source": "env"}
        else:
            out[k] = {"value": env_val, "configured": bool(env_val), "source": "env"}

    # ENABLED_TOOLS is application data, not an environment variable.
    try:
        db = await _adb()
        res = await db.table("settings").select("value").eq("key", "ENABLED_TOOLS").maybe_single().execute()
        et = res.data["value"] if (res and res.data) else ""
    except Exception:
        et = ""
    out["ENABLED_TOOLS"] = {"value": et, "configured": bool(et), "source": "db"}
    return out


async def save_settings(data: dict) -> None:
    """
    Persist ONLY application-data settings to Supabase. Any environment-config
    key (credentials/service settings) is silently ignored so the VPS env stays
    ...
    rows = [
        ...
        if v is not None and v != "" and k not in ENV_CONFIG_KEYS
    ]
    if rows:
...
async def get_setting(key: str, default: str = "") -> str:
    """
    Environment-config keys come ONLY from os.environ.
    Application-data keys (system_prompt, ENABLED_TOOLS, …) come from Supabase.
    """
...
async def set_setting(key: str, value: str) -> None:
    # Environment-config keys are read-only (VPS env is the single source of truth).
    if key in ENV_CONFIG_KEYS:
        raise ValueError(f"'{key}' is an environment variable and cannot be set via the database.")
    db = await _adb()
